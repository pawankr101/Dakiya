...
export class HttpServer<hv extends HttpVersion = 'http1', hs extends HttpSecurity = 'http'> {
    ...
    private constructor(httpVer: hv, httpSec: hs, options?: ServerOptions<hv,hs>, privateHash?: string) {
        if(!privateHash || privateHash!==HttpServer.#staticHash) throw new Exception(`'HttpServer' class constructor can not be called from outside. use 'HttpServer.build()' static method to get the singleton instance of this class.`, { code: 'DAKIYA_HTTP_SERVER_ERROR' });
    ...
    start(options: ListenOptions, cb: StartCallbacks) {
        if(cb.onError) {
            this.#server.on('error', (error) => {
                cb.onError(error.code === 'EADDRINUSE'
                    ? new Exception(`Port ${options.port} is already occupied.`, { cause: error, code: 'DAKIYA_HTTP_SERVER_ERROR' })
                    : new Exception(error, { code: 'DAKIYA_HTTP_SERVER_ERROR' })
                );
            });
        }
