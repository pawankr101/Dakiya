...
export const createDbIfNotExists = async () => {
    const { host, port, user, password, database, deleteOldDatabase } = DB;
...
export const initTables = async (connection: Sql) => {
    ...
    await createUserRelationshipsTable(connection);
    ...
    await createMessageExclusionsTable(connection);
    ...
    try {
        await connection`
            CREATE TABLE IF NOT EXISTS message_reactions (
                ...
                -- Store root_id for fast thread queries, but do not use a foreign key constraint
                message_root_id UUID NOT NULL,

                -- This handles your referential integrity
