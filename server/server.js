...
const CONFIG = {
    ...
    envFile: resolvePath(CWD, "env", ".env.dev"),
    watcherDelay: 2000
};
/********************* CONFIGURATIONS: End *********************/
...
/************************ Types: Start ************************/
/**
 * @typedef { import("./src/env.d.ts").Env } Env
 * @typedef {{ outdir: string, entryPoints: [{in: string, out: string}], alias: {[key: string]: string}, tsconfig: string, target: Array<string>, isProd?: boolean }} BuilderOptions
 * @typedef {{ envFile: string, watchDir?: string, watcherDelay?: number }} AppRunnerOptions
 */
/************************ Types: End *************************/
...
class ProjectBuilder {
     ...
     */
    constructor(privateHash,{ outdir, entryPoints, alias, tsconfig, target, isProd}) {
        ...
        this.buildOptions = {
            ...
            outdir, entryPoints, alias, tsconfig, target,
            minify: isProd,
            treeShaking: isProd,
            write: true,
            sourcemap: 'inline'
    ...
    async build() {
        try {
            ...
            await esbuild(this.buildOptions);
        } catch (error) {
...
class AppRunner {
    ...
    constructor(privateHash, builderOptions, runnerOptions) {
        ...
        this.#envFile = runnerOptions.envFile;
