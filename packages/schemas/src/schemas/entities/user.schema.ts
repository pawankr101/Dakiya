import { Type } from 'typebox';
import { AccountStatusSchema, BackupFrequencySchema, DBTableSchema, EpochTimestampSchema, FontSizeSchema, GenderSchema, JsonBSchema, PlatformSchema, PrivacyLevelSchema, ThemeSchema, UUIDSchema } from '../utils';

export const UserSchema = DBTableSchema({
    username: Type.String(),
    mobile: Type.String(),
    email: Type.Optional(Type.String({ format: 'email' })),

    password: Type.Optional(Type.String()),

    name: Type.Optional(Type.String()),
    dob: Type.Optional(Type.String({ format: 'date' })),
    gender: Type.Optional(GenderSchema),
    country: Type.Optional(Type.String()),
    isVerified: Type.Boolean({ default: false }),

    dp: Type.Optional(Type.String( { format: 'uri' })),
    bio: Type.Optional(Type.String()),

    isDeleted: Type.Boolean({ default: false }),
    deletedAt: Type.Optional(EpochTimestampSchema)
}, { $id: 'UserSchema' });

export const LabelsAndCirclesSchema = Type.Array(Type.Object({
    id: UUIDSchema,
    name: Type.Optional(Type.String()),
    position: Type.Number()
}), { maxItems: 8, default: [], $id: 'LabelsAndCirclesSchema' });

export const UserSettingsNotificationsSchema = Type.Object({
    email: Type.Boolean({ default: false })
}, { $id: 'UserSettingsNotificationsSchema' });

export const UserSettingsPrivacySchema = Type.Object({
    readReceipts: Type.Boolean({ default: true }),
    lastSeen: PrivacyLevelSchema,
    email: PrivacyLevelSchema,
    dp: PrivacyLevelSchema,
    dob: PrivacyLevelSchema,
    bio: PrivacyLevelSchema
}, { $id: 'UserSettingsPrivacySchema' });

export const UserSettingsBackupSchema = Type.Object({
    enabled: Type.Boolean({ default: false }),
    provider: Type.Optional(Type.String()),
    backupLocation: Type.Optional(Type.String()),
    backupFrequency: BackupFrequencySchema,
    overWifiOnly: Type.Boolean({ default: true }),
    lastBackupAt: Type.Optional(EpochTimestampSchema)
}, { $id: 'UserSettingsBackupSchema' });

export const UserSettingsAccountSchema = Type.Object({
    accountStatus: AccountStatusSchema,
    twoFactorAuth: Type.Boolean({ default: false }),
    mfa: Type.Object({
        sms: Type.Optional(Type.String()),
        email: Type.Optional(Type.String()),
        totp: Type.Optional(Type.String())
    }),
    recoveryCodesHash: Type.Optional(Type.Array(Type.String())),
    lastPasswordChange: Type.Optional(EpochTimestampSchema)
}, { $id: 'UserSettingsAccountSchema' });

export const UserSettingsSchema = DBTableSchema({
    userId: UUIDSchema,

    language: Type.String({ default: 'en' }),
    timezone: Type.String({ default: 'UTC' }),

    pinnedConversationId: Type.Optional(UUIDSchema),
    labels: JsonBSchema(LabelsAndCirclesSchema, '[]'),
    circles: JsonBSchema(LabelsAndCirclesSchema, '[]'),
    notifications: JsonBSchema(UserSettingsNotificationsSchema),
    privacy: JsonBSchema(UserSettingsPrivacySchema),
    backup: JsonBSchema(UserSettingsBackupSchema),
    account: JsonBSchema(UserSettingsAccountSchema),
    lastActiveAt: Type.Optional(EpochTimestampSchema)
}, { $id: 'UserSettingsSchema' });

export const DeviceChatPrefsSchema = Type.Object({
    theme: ThemeSchema,
    fontSize: FontSizeSchema,
    mediaAutoDownload: Type.Object({
        photos: Type.Boolean({ default: true }),
        videos: Type.Boolean({ default: true }),
        audio: Type.Boolean({ default: true }),
        documents: Type.Boolean({ default: true })
    })
}, { $id: 'DeviceChatPrefsSchema' });

export const DeviceNotificationPrefsSchema = Type.Object({
    enabled: Type.Boolean({ default: true }),
    groupNotifications: Type.Boolean({ default: true }),
    vibration: Type.Boolean({ default: true }),
    sound: Type.Boolean({ default: true })
}, { $id: 'DeviceNotificationPrefsSchema' });

export const DeviceSchema = DBTableSchema({
    userId: UUIDSchema,
    clientId: Type.String(),
    deviceName: Type.Optional(Type.String()),
    platform: PlatformSchema,
    osName: Type.Optional(Type.String()),
    appVersion: Type.Optional(Type.String()),
    userAgent: Type.Optional(Type.String()),
    ipAddress: Type.Optional(Type.String()),
    fcmToken: Type.Optional(Type.String()),
    isActive: Type.Boolean({ default: true }),
    lastActiveAt: Type.Optional(EpochTimestampSchema),
    chatPrefs: JsonBSchema(DeviceChatPrefsSchema),
    notificationPrefs: JsonBSchema(DeviceNotificationPrefsSchema)
}, { $id: 'DeviceSchema' });

export const UserRelationshipSchema = DBTableSchema({
    ownerId: UUIDSchema,
    userId: UUIDSchema,
    isContact: Type.Boolean({ default: false }),
    isFavorite: Type.Boolean({ default: false }),
    isBlocked: Type.Boolean({ default: false }),
    circleIds: Type.Array(UUIDSchema, { maxItems: 4, default: [] })
}, { $id: 'UserRelationshipSchema' });
